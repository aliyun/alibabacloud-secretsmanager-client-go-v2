...
func (cl *CommonLogger) parseExceptionErrorMsg(format string, params ...interface{}) string {
	if len(params) > 0 {
		...
		case *tea.SDKError:
			format = format + fmt.Sprintf("\tmodeName:%s\terrorCode:%s\terrMsg:%s\tcaused by:%+v", cl.modeName, tea.StringValue(err.Code), tea.StringValue(err.Message), err)
		}
	}
	return format
}
