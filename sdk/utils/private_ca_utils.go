...
// GetCaExpirationUtcDate 获取CA证书的过期时间(UTC格式)
//
// @param caContent ca文件内容
// @return CA证书的过期时间，格式为yyyy-MM-dd UTC
func GetCaExpirationUtcDate(caContent string) string {
	if caContent == "" {
		return ""
	}
	certContent := TrimCaContent(caContent)
	certBytes, err := base64.StdEncoding.DecodeString(certContent)
	if err != nil {
		block, _ := pem.Decode([]byte(caContent))
		if block == nil {
			return ""
		}
	...
	return expirationDate.UTC().Format("2006-01-02")
}

// TrimCaContent 清理CA内容，提取证书部分
func TrimCaContent(caContent string) string {
	lastCertContent := caContent
	lastBeginIndex := strings.LastIndex(caContent, BeginCertificate)
	if lastBeginIndex > 0 {
		lastCertContent = caContent[lastBeginIndex:]
	}
	result := strings.Replace(lastCertContent, BeginCertificate, "", -1)
	result = strings.Replace(result, EndCertificate, "", -1)
